...
from typing import List, Any, Awaitable, Dict

from GPTParser.defines import Dict
from .model import Message, MessagePrompt, SortingPrompt, CleaningPrompt, Attributes
from .defines import *
import asyncio
...
class InterestParser(Parser):
    ...
    async def __call__(self, kwargs: Dict[str, Any]) -> str:
        if STR_MESSAGES not in kwargs: return ""
        interests = []
...
class GPTParser(Parser):
    ...
    def create_chat(self):
        return ChatOpenAI(model="gpt-3.5-turbo-16k-0613", openai_api_key=dotenv.dotenv_values()['OPENAI_API_KEY'])
    ...
    def create_prompt(self):
        ...
        return ChatPromptTemplate.from_messages(
            [from_template[template.role](template.content) for template in self.templates]
        )
...
class FormattedOutputParser(GPTParser):
    ...
    def create_chain(self):
        return create_structured_output_chain(Attributes, self.chat, self.prompt)
