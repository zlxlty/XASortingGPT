    ...
    with open('/'.join([INPUT_FOLDER, CSV_FILENAME]), 'r') as file:
        ...
        while True:
            ...
            for user_id, row in next_batch:
                ...
                sortingHat = SortingHat.put_on(user_id, username)
                sortingHat.read_mind(message_segments, db)
                
                sorting_tasks.append(sortingHat.decide())
            ...
            for index in rows_to_retry:
                ...
                if retries_map[next_batch[index][0]] > max_retries:
                    print(f"User {next_batch[index][0]} failed to be sorted after {max_retries} retries")
                    continue
